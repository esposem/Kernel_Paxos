...
static void client_submit_value(struct client *c) {
  ...
  paxos_log_info("Client submitted PAXOS_CLIENT_VALUE len %zu value %s",
                 v->size, v->value);
}
...
static void update_stats(struct stats *stats, struct client_value *delivered,
                         size_t size) {
  struct timeval tv;
  do_gettimeofday(&tv);
...
static void on_deliver(unsigned iid, char *value, size_t size, void *arg) {
  ...
  if (v->client_id == c->id) {
    ...
    // printk(KERN_INFO "%s On deliver iid:%d value:%.16s",kclient->name, iid,
    // v->value );
  }
}
...
static struct client *make_client(int proposer_id, int outstanding,
                                  int value_size) {
  ...
  c->learner = evlearner_init(on_deliver, c, if_name);
  if (c->learner == NULL) {
